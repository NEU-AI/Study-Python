...
class calculator():
    def __init__(self):
        ...
        # 计算器变量
        self.l_operand = 0  #左操作数 为连加使用
        self.r_operand = 0  #右操作数 为连加使用
        self.operator = 123 #操作符
        self.result = 0     #结果
        self.dotFlag = 0	#小数点标志
        self.dotLoc = 0     #小数点后多少个位置
    ...
    def sendData(self, num):
        # 数字0-9
        if num < 10:
            if self.operator == 11:#如果按了=把操作符设置成+，右操作数置空
                self.r_operand = 0
                self.operator = 123
            if self.dotFlag == 0:
                self.r_operand = self.r_operand * 10 + num
            else:#有小数点时操作方式当按过小数点后每次输入的数字都是小数点后dotloc位的数
